...
void DesktopWindow::close() {
    if (renderTimer) KillTimer(hwnd, renderTimer);
    if (updateTimer) KillTimer(hwnd, updateTimer);
    if (desktopCheckTimer) KillTimer(hwnd, desktopCheckTimer);
    removeTrayIcon();
    if (hwnd) {
        DestroyWindow(hwnd);
        hwnd = NULL;
...
void DesktopWindow::render() {
    if (!visible || !renderer || !hwnd) return;
...
LRESULT CALLBACK DesktopWindow::windowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    if (window) {
        switch (msg) {
            case WM_PAINT:      window->onPaint(); return 0;
            case WM_TIMER:      window->onTimer(); return 0;
            case WM_MOUSEMOVE:  window->onMouseMove(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)); return 0;
            case WM_LBUTTONDOWN:window->onMouseDown(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)); return 0;
            case WM_LBUTTONUP:  window->onMouseUp(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)); return 0;
            case WM_MOUSEWHEEL:
                if (window->renderer) {
                ...
                return 0;
            case WM_KEYDOWN:    window->onKeyDown(wParam); return 0;
            case WM_COMMAND:    window->onCommand(wParam); return 0;
            case WM_APP+1:
...
void DesktopWindow::onKeyDown(WPARAM key) {
    ...
    else if (key == VK_F5) update();
    else if (key == VK_SPACE && renderer) renderer->toggleAudioPlayback();
    else if (key == VK_RIGHT && renderer) renderer->playNextTrack();
    else if (key == VK_LEFT && renderer) renderer->playPreviousTrack();
...
bool DesktopWindow::checkIfOnDesktop() {
    ...
    if (IsIconic(fg) || !IsWindowVisible(fg)) return true;
...
void DesktopWindow::setSize(int width, int height) {
    ...
    if (hwnd) {
        ...
        if (renderer) renderer->resize(windowWidth, windowHeight);
    }
    Config& cfg = Config::getInstance();
