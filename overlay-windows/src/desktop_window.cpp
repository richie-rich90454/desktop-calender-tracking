...
namespace CalendarOverlay{
    ...
    void DesktopWindow::render(){
        ...
        if (doubleBufferDC&&doubleBufferBitmap){
            ...
            AlphaBlend(screenDC, 0, 0, clientRect.right, clientRect.bottom, doubleBufferDC, 0, 0, clientRect.right, clientRect.bottom, blend);
    ...
    void DesktopWindow::onKeyDown(WPARAM key){
        if (key==VK_ESCAPE){
            ...
            if (renderer){
                float currentVolume=renderer->getAudioVolume();
                renderer->setAudioVolume(std::min(1.0f, currentVolume+0.1f));
            }
        }
        else if (key==VK_DOWN){
            if (renderer){
                float currentVolume=renderer->getAudioVolume();
                renderer->setAudioVolume(std::max(0.0f, currentVolume-0.1f));
            }
        }
        else if (key=='M'||key=='m'){
            if (renderer){
                ...
                if (currentVolume>0.0f){
                    renderer->setAudioVolume(0.0f);
                }
                else{
                    renderer->setAudioVolume(0.5f);
                }
            }
        }
    }
    ...
    bool DesktopWindow::checkIfOnDesktop(){
        ...
        if (strcmp(className, "Progman")==0||strcmp(className, "WorkerW")==0||strcmp(className, "Shell_TrayWnd")==0 |strcmp(className, "Button")==0){
            return true;
        }
