...
public class AudioPlayerFrame extends JPanel {
    ...
    private static final Color ACCENT_COLOR = new Color(40, 167, 69); // Green accent for play button
    ...
    private void setupLayout() {
        ...
        // Action Buttons Panel (Upload/Delete/Clear)
        ...
        // Progress Panel (with time labels)
        ...
        // CRITICAL FIX: Single bottom container using BoxLayout to avoid BorderLayout conflicts
        ...
        // Assemble main layout - NO CONFLICTING REGIONS
        ...
        if (selected != null) {
            ...
            if (success) {
                ...
                JOptionPane.showMessageDialog(this,
                    "Could not play audio file. Format may not be supported.",
                    "Playback Error",
    ...
    private void uploadAudioFile() {
        ...
        fileChooser.addChoosableFileFilter(new FileFilter() {
            ...
            public boolean accept(File f) {
                ...
                return name.endsWith(".mp3") || name.endsWith(".wav") || name.endsWith(".mid")
                    || name.endsWith(".midi") || name.endsWith(".ogg") || name.endsWith(".flac");
            }
            @Override
    ...
    private void deleteSelectedTrack() {
        ...
        if (selected != null) {
            int confirm = JOptionPane.showConfirmDialog(SwingUtilities.getWindowAncestor(this), 
                "Delete track '" + selected.getDisplayName() + "'?", 
                "Confirm Delete", 
    ...
    private void clearAllTracks() {
        if (!playlist.isEmpty()) {
            int confirm = JOptionPane.showConfirmDialog(SwingUtilities.getWindowAncestor(this), 
                "Delete all " + playlist.size() + " tracks?", 
                "Confirm Clear All", 
                JOptionPane.YES_NO_OPTION);
            if (confirm == JOptionPane.YES_OPTION) {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            // Set icon on JFrame with robust fallback handling
    ...
    /**
     * Sets application icon with robust multi-strategy fallback handling.
     */
    private static void setApplicationIcon(JFrame frame) {
        Image icon = null;
        
        // Strategy 1: Load from classpath root (/icon.png)
        URL iconUrl = AudioPlayerFrame.class.getResource("/icon.png");
        if (iconUrl != null) {
            icon = Toolkit.getDefaultToolkit().createImage(iconUrl);
        }
        
        // Strategy 2: Fallback to file system if classpath fails
        ...
        // Strategy 3: Create a simple fallback icon if all else fails
        if (icon == null || icon.getWidth(null) == -1) {
        ...
        // Wait for image to load before setting
    ...
    /**
     * Creates a professional fallback icon (green circle with musical note)
     */
    private static Image createFallbackIcon() {
        ...
        GradientPaint gradient = new GradientPaint(
            size/2f, size/2f, new Color(40, 167, 69),
            size/2f, size, new Color(30, 136, 56)
        );
    ...
    private static class AudioTrackCellRenderer extends JPanel implements ListCellRenderer<AudioTrack> {
        ...
        @Override
        public Component getListCellRendererComponent(JList<? extends AudioTrack> list, 
                AudioTrack track, int index, boolean isSelected, boolean cellHasFocus) {
