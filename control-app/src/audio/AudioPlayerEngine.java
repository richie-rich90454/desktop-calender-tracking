...
public class AudioPlayerEngine implements AutoCloseable{
    ...
    private boolean isSameTrackResume;
    ...
    public boolean play(AudioTrack track){
        ...
        if (currentTrack!=null&&currentTrack.equals(track)&&playbackState==PlaybackState.PAUSED){
            return resume();
        }
        stop();
        currentTrack=track;
    ...
    private boolean playMp3WithSystemCommand(AudioTrack track) throws IOException{
        ...
        if (os.contains("win")){
            ...
            pb=new ProcessBuilder("powershell", "-c","$player = New-Object -ComObject WMPlayer.OCX; "+"$player.URL = '"+path+"'; "+"while ($player.playState -ne 1) { Start-Sleep -Milliseconds 100 }; "+"$player.close()");
        }
    ...
    public long getCurrentPosition(){
        ...
        try{
            return switch (currentTrack.getFileExtension()){
                case "mp3"->audioClip!=null?audioClip.getMicrosecondPosition()/1000:(playbackState==PlaybackState.PAUSED?resumePosition:0);
