...
/*
*Abstract base implementation for AI clients using HTTP.
 *
*Responsibilities:
*- Handle HTTP communication with AI APIs
...
*- Duration for timeout management
*- DateTimeFormatter for date parsing
 *
*Design intent:
*Concrete providers extend this class and implement format-specific methods.
*All HTTP communication goes through this base class.
*Usage tracking is automatic for all API calls.
 */
public abstract class BaseAIClient implements AIClient{
    ...
    protected static final DateTimeFormatter TIME_FORMATTER=DateTimeFormatter.ISO_LOCAL_TIME;
    ...
    protected String sendRequest(String requestBody) throws AIException{
        ...
        for (int attempt=1;attempt<=maxRetries;attempt++){
            try{
                ...
                if (response.statusCode()==200){
                    ...
                    throw new AIException("Invalid API key", AIException.ErrorType.AUTHENTICATION_ERROR);
                }
                    ...
                    throw new AIException("Server error: "+response.statusCode(),  AIException.ErrorType.SERVER_ERROR);
                }
                else{
                    throw new AIException("API error: "+response.statusCode()+" - "+response.body(), AIException.ErrorType.OTHER);
                }
            }
            catch (InterruptedException e){
                Thread.currentThread().interrupt();
                throw new AIException("Request interrupted", AIException.ErrorType.OTHER, e);
            }
            catch (Exception e){
                if (attempt==maxRetries){
                    throw new AIException("Request failed after "+maxRetries+" attempts: "+e.getMessage(), AIException.ErrorType.NETWORK_ERROR, e);
                }
                try{
                    Thread.sleep(retryDelay*attempt);
                }
        ...
        throw new AIException("Request failed after "+maxRetries+" attempts", AIException.ErrorType.NETWORK_ERROR);
    }
    ...
    protected Event createEvent(String title, String dateStr, String startTimeStr, String endTimeStr) throws AIException{
        try{
            ...
            throw new AIException("Failed to create event from: "+title+", "+dateStr+", "+startTimeStr+"-"+endTimeStr, AIException.ErrorType.INVALID_RESPONSE, e);
        }
    }
